...
var new_data=[]; 
 data1.forEach(function(arr,index){
...
if(index==0){
  ...
  var ss=[];
ss.push(arr[0]);
 var new_ss=arr.slice(1);
 
 new_ss.forEach(function(arrs,index){
 ...
 var data = google.visualization.arrayToDataTable(new_data);
  ...
  chart.draw(view.toDataTable(), {
    ...
    function getTooltip(dt, row, col) { 
      var get_el=data1[row+1][col];
    var tooltip = '<div class="tooltip">';
    tooltip += '<div><h3>' + dt.getColumnLabel(col) +'</h3> </div>';
    tooltip += '<div><strong>Pre. Rate=</strong> ' + get_el[1] + ' </div>';
    tooltip += '<div><strong>Cur. Rate=</strong>' + get_el[2] + '</div>';   

    tooltip += '<div><strong>Percent=</strong>' + dt.getFormattedValue(row, col) + '</div>';
       tooltip += '<div ><strong>Date= </strong>'+ dt.getFormattedValue(row, 0) + '</div>';
...
head_arr.forEach( function(element, index) {
 ...
 var all_ss=[] ;
all_ss.push(el.percent,el.c_rate,el.p_rate);
body_arr[date_arr.indexOf(el.dates)][get_ind]=all_ss;
